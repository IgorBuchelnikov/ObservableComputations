...
namespace ObservableComputations
{
	...
	public class Dictionaring<TSourceItem, TKey, TValue> : IDictionary<TKey, TValue>, IHasSources, IComputing, INotifyMethodChanged, ISourceItemKeyChangeProcessor, ISourceItemValueChangeProcessor, ISourceCollectionChangeProcessor
	{
		...
		internal void ValidateInternalConsistency()
		{
			...
			if (_itemInfos.Count != source.Count) throw new ValidateInternalConsistencyException("Consistency violation: Dictionaring.1");
			if (Count != source.Count) throw new ValidateInternalConsistencyException("Consistency violation: Dictionaring.16");
			...
			if (source != null)
			{
				if (_sourcePositions.List.Count != source.Count)
					throw new ValidateInternalConsistencyException("Consistency violation: Dictionaring.15");
				...
				for (int sourceIndex = 0; sourceIndex < source.Count; sourceIndex++)
				{
					...
					if (!ContainsKey(key))
						throw new ValidateInternalConsistencyException("Consistency violation: Dictionaring.2");
					...
					if (!this[key].IsSameAs(value))
						throw new ValidateInternalConsistencyException("Consistency violation: Dictionaring.3");

					if (_sourcePositions.List[sourceIndex].Index != sourceIndex) throw new ValidateInternalConsistencyException("Consistency violation: Dictionaring.4");
					if (itemInfo.KeyExpressionWatcher._position != _sourcePositions.List[sourceIndex]) throw new ValidateInternalConsistencyException("Consistency violation: Dictionaring.5");
					if (itemInfo.ValueExpressionWatcher._position != _sourcePositions.List[sourceIndex]) throw new ValidateInternalConsistencyException("Consistency violation: Dictionaring.6");

					if (!_sourcePositions.List.Contains((KeyValueExpressionItemInfo<TKey, TValue>) itemInfo.KeyExpressionWatcher._position))
						throw new ValidateInternalConsistencyException("Consistency violation: Dictionaring.7");

					if (!_sourcePositions.List.Contains((KeyValueExpressionItemInfo<TKey, TValue>) itemInfo.ValueExpressionWatcher._position))
						throw new ValidateInternalConsistencyException("Consistency violation: Dictionaring.8");

					if (itemInfo.KeyExpressionWatcher._position.Index != sourceIndex)
						throw new ValidateInternalConsistencyException("Consistency violation: Dictionaring.17");

					if (itemInfo.ValueExpressionWatcher._position.Index != sourceIndex)
						throw new ValidateInternalConsistencyException("Consistency violation: Dictionaring.18");

					if (!itemInfo.Key.IsSameAs(key))
						throw new ValidateInternalConsistencyException("Consistency violation: Dictionaring.10");

					if (!itemInfo.Value.IsSameAs(value))
						throw new ValidateInternalConsistencyException("Consistency violation: Dictionaring.9");
				}
			}			
		}
