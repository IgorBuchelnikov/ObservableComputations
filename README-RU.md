...
[ReactiveUI](https://github.com/reactiveui/ReactiveUI) и [DynamicData](https://github.com/reactiveui/DynamicData) делают примерно то же самое, что и ObservableComputations, но ObservableComputations менее многословна, проще в использовании, меньше обращается к исходным данным. Наборы функций этих библеотек и ObservableComputations пересекаются. Набор функций этих библеотек шире и более низкоуровный, чем набор функций ObservableComputations. Эти библеотеки позволяют производить тонкую настройку, но основные функции Вашего приложения могут быть реализованы с помощью ObservableComputations проще и изящнее. Вы можете сравнить эти библеотеки и ObservableComputations в действии, см. [ObservableComputations.Samples](https://github.com/IgorBuchelnikov/ObservableComputations.Samples).
...
Мы продолжаем рассматривать пример из [предыдущего раздела](#передача-аргументов-как-обозреваемых). Мы использовали следующий код для того чтобы отследить измененич в *hockeyTeamManager.HockeyTeamInterested*:
```csharp
new Computing<ObservableCollection<Person>>(
    () => hockeyTeamManager.HockeyTeamInterested)
```
...
Если после инстанцирования класса вычисления коллекции (напр. Filtering), ожидается что коллекция значительно вырастет, имеет смысл пеердать в конструктор аргумент *capacity*, чтобы зарезервировать память под коллекцию. 
...
### Укоротите Ваш код
См. [здесь](#передача-аргументов-как-обозреваемых) and [здесь](#gердача-коллекции-источника-как-обозваемого-аргумента).
...
Класс и [метод расширения](https://docs.microsoft.com/en-us/dotnet/csharp/programming-guide/classes-and-structs/extension-methods) Binding позволяет связать два произвольных выражения. Первое выражение это источник. Второе выражение является целевым. Когда значение выражения источника меняется, новое значение присваивается целевому выражению:  
